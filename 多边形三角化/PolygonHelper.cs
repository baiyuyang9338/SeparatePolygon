    ...
    static class PolygonHelper {
        ...
        public static List<int> Resolve(List<Vec> polygon) {
            ...
            for (int i = 0; i < polygon.Count; i++) {
                ...
                // 确保顺序为顺时针，逆时针则反向插入
                if (isCW) pointStatuses.AddLast(pointStatus);
                else pointStatuses.AddFirst(pointStatus);
            }
            // 可分离的点
            ...
            for (LinkedListNode<PointStatus> node = pointStatuses.First; node != null; node = node.Next) {
                ...
                if (node.Value.isSeparable) separablePointStatuses.AddFirst(node);
            }
            // 开始分离
            while (pointStatuses.Count >= 3) {
                if (separablePointStatuses.Count == 0) return null; // 分离失败，例如自身相交情况
                ...
                if (prev.Value.isSeparable) {
                    ...
                    if (!prev.Value.isSeparable) separablePointStatuses.Remove(prev);
                } else {
                    UpdatePointStatus(prev);
                    if (prev.Value.isSeparable) separablePointStatuses.AddFirst(prev);
                }
                if (next.Value.isSeparable) {
                    UpdatePointStatus(next);
                    if (!next.Value.isSeparable) separablePointStatuses.Remove(next);
                } else {
                    UpdatePointStatus(next);
                    if (next.Value.isSeparable) separablePointStatuses.AddFirst(next);
                }
            }
            return tris;
        }
